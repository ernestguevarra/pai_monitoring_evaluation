...
email_progress_report <- function(message = email_report_message,
  ...
  blastula::add_attachment(
    ...
    blastula::smtp_send(
      ...
      from = sender, 
      subject = paste0(
        "Daily survey progress report - CMAM Coverage S3M Survey - ",
        Sys.Date()
      ), 
...
email_coverage_dashboard <- function(message = email_dashboard_message,
                                     #attachment = survey_progress_report[1],
                                     sender = Sys.getenv("GMAIL_USERNAME"),
                                     recipient = Sys.getenv("REPORT_RECIPIENTS")) {
  blastula::smtp_send(
    ...
    from = sender, 
    subject = paste0(
      "CMAM Coverage Dashboard - ", Sys.Date()
    ), 
    credentials = blastula::creds_envvar(
      user = sender, 
      pass_envvar = "GMAIL_PASSWORD", 
